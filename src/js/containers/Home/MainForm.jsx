...
class MainForm extends Component {
  ...
  onFormSubmit(event) {
    ...
    if (this.state.expID !== '' && this.state.partID !== '') {
      this.props.storeExpData(this.state.expID, this.state.partID);
      this.props.formSubmitted();
    } else {
      this.setState({
        alertMessage: <Alert color="danger">Please fill experiment and participant ID</Alert>,
      });
    }
  }
  ...
  render() {
    return (
      <Form onSubmit={this.onFormSubmit}>
        ...
        <FormGroup>
          <Button className="mt-4" size="lg" color="success">Submit</Button>
        </FormGroup>
